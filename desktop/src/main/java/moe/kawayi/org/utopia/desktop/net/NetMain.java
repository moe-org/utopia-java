...
public final class NetMain {
     ...
     */
    @Nullable
    private static final AtomicReference<NioEventLoopGroup> WORKER_GROUP = new AtomicReference<>(null);
    ...
    public static synchronized void start(@NotNull String uriPath)
        ...
        bootstrap.group(WORKER_GROUP.get()).channel(NioSocketChannel.class).handler(new NettyChannelInit());
    ...
    public static void close() {
        if (!IS_RUNNING.getAndSet(false)) return;

        var got = WORKER_GROUP.getAndSet(null);

        if (got != null && (!(got.isTerminated() || got.isShutdown()))) got.shutdownGracefully();
    }
}
