...
namespace {
	string truecolor(unsigned char r, unsigned char g, unsigned char b) {
		ostringstream os;
		os << "\x1b[38;2;" << to_string(r) << ";" << to_string(g) << ";"
		   << to_string(b) << "m";
		return os.str();
	}
	...
	string on_truecolor(unsigned char r, unsigned char g, unsigned char b) {
		ostringstream os;
		os << "\x1b[48;2;" << to_string(r) << ";" << to_string(g) << ";"
		   << to_string(b) << "m";
		return os.str();
	}
	...
	inline void print_pink(std::string str) {
		cout << truecolor(0, 0, 0) << on_truecolor(245, 171, 185) << str
			 << clear_style << "\n";
	}

	inline void print_white(std::string str) {
		cout << truecolor(0, 0, 0) << on_truecolor(255, 255, 255) << str
			 << clear_style << "\n";
	}
} // namespace
...
namespace utopia {
	...
	void print_logo() {
		...
		print_blue(
			"| |  | | |            (_)        / ____|                         ");
		print_blue(
			"| |  | | |_ ___  _ __  _  __ _  | (___   ___ _ ____   _____ _ __ ");
		print_blue(
		...
		print_pink(
			" \\____/ \\__\\___/| .__/|_|\\__,_| |_____/ \\___|_|    \\_/ \\___|_|   ");
		print_pink(
			"                | |                                              ");
		print_pink(
			"                |_|                                              ");
