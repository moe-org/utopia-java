...
public class HoconConfigTest {
    ...
    public void parseTest() throws Exception {
        ...
        Assertions.assertEquals(
                (byte) 1, hocon.getByte(hocon.createPath("byte")).orElseThrow());
        Assertions.assertEquals(
                (short) 2, hocon.getShort(hocon.createPath("short")).orElseThrow());
        ...
        Assertions.assertEquals(
                6.0d, hocon.getDouble(hocon.createPath("double")).orElseThrow());
        Assertions.assertEquals(
                false, hocon.getBoolean(hocon.createPath("boolean")).orElseThrow());
        Assertions.assertEquals(
                "Hello HOCON!", hocon.getString(hocon.createPath("string")).orElseThrow());
        ...
        Assertions.assertEquals(
                true, object.getBoolean(object.createPath("in-list")).orElseThrow());

        object = hocon.getObject(hocon.createPath("object")).orElseThrow();

        Assertions.assertEquals(
                true, object.getBoolean(object.createPath("in-object")).orElseThrow());
    }
}
